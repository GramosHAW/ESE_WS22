...
#ifdef SIM_PUSHER
Simulation::Simulation(SimItemHandling* itemhandling, SimHCI *hci, ISimInitCompleteObserver *ich) : feedpusher(&shadow), drive(&shadow), siminitobserver(ich) {
#else
Simulation::Simulation(SimItemHandling* itemhandling, SimHCI *hci, ISimInitCompleteObserver *ich) : feedseparator(&shadow), drive(&shadow), siminitobserver(ich) {
#endif
